...
def part1(data):
	...
	ret = [ score_map[i] for i in ret if i ]
	
	return sum(ret)

def completion_chunk(chunk, token_pairs):
	open_tokens = []
	for ch in chunk:
		if ch in token_pairs.keys():
			open_tokens.append(ch)
...
def part2(data):
	...
	not_corrupted_lines = [ line for line in data if not parse_chunk(line, token_pairs) ]
	completion_lists = [completion_chunk(chunk, token_pairs) for chunk in not_corrupted_lines]
